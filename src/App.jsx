...
const BRIEFING={date:"2026년 6월 2일 · 오전 10:00",headline:"美 국채 금리 안정세, 안전자산 선호 지속",body:"간밤 글로벌 증시는 혼조세를 보였습니다. 오늘의 핵심 개념은 '금리와 채권 가격의 관계'입니다.",term:{word:"유동성 (Liquidity)",def:"자산을 가격 손실 없이 빠르게 현금으로 바꿀 수 있는 정도. 유동성이 낮으면 팔고 싶을 때 제값을 못 받을 수 있습니다."}};
...
function Paywall({onClose,onSubscribe,context}){
  ...
  return <div style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.6)",backdropFilter:"blur(4px)",zIndex:50,display:"flex",alignItems:"flex-end",animation:"rise .25s ease both"}} onClick={onClose}>
    <div onClick={e=>e.stopPropagation()} style={{width:"100%",background:`linear-gradient(180deg,${C.card2},${C.card})`,borderRadius:"24px 24px 0 0",border:`1px solid ${C.borderStrong}`,borderBottom:"none",padding:"20px 18px 22px",maxHeight:"88%",overflowY:"auto"}} className="sc">
      ...
      <div style={{...B,fontSize:10.5,color:C.faint,marginTop:10,textAlign:"center",lineHeight:1.5}}>언제든 해지 가능 · 데모 결제</div>
...
function Learn({premium,setPremium}){
  ...
  let body;
  if(screen.type==="home") body=<LearnHome onSelectSeason={id=>setScreen({type:"season",seasonId:id})} progress={progress} currentSeason={currentSeason} premium={premium} onUpgrade={()=>setPaywall({context:null})}/>;
  else if(screen.type==="season"){
    const season=SEASONS.find(s=>s.id===screen.seasonId);
    body=<SeasonView season={season} onBack={()=>setScreen({type:"home"})} onSelectSeries={id=>setScreen({type:"series",seasonId:screen.seasonId,seriesId:id})} progress={progress}/>;
  }
  else if(screen.type==="series"){
    const season=SEASONS.find(s=>s.id===screen.seasonId);
    const sr=season.series.find(s=>s.id===screen.seriesId);
    if(screen.lessonIdx!==undefined){
      body=<LessonView lesson={sr.lessons[screen.lessonIdx]} idx={screen.lessonIdx} total={sr.lessons.length} color={season.color}
      ...
      body=<SeriesView series={sr} seriesId={sr.id} color={season.color} completed={progress[sr.id]||0} premium={premium}
...
function About({premium}){
  ...
  return <div className="sc" style={{overflowY:"auto",height:"100%",animation:"rise .4s ease both"}}>
    ...
    <div style={{margin:"12px 16px",borderRadius:18,padding:18,background:`linear-gradient(135deg,${C.purpleDeep},${C.purple})`,position:"relative",overflow:"hidden"}}>
